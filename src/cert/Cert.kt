...
object Cert {
    ...
    fun buildSelfSigned(
        ...
        val rawCert = Der.sequence(
            ...
            subject.encoded,
            pub.encoded,
            Der.contextSequence(3, Der.sequence(
                Der.sequence(SUBJECT_KEY_ID_OID, Der.octetString(Der.octetString(pubKeyHash))),
                Der.sequence(AUTHORITY_KEY_ID_OID, Der.octetString(Der.sequence(Der.contextTag(0, pubKeyHash)))),
                Der.sequence(BASIC_CONSTRAINTS_OID, Der.booleanTrue(), Der.octetString(Der.sequence(Der.booleanTrue()))),
                Der.sequence(SUBJECT_ALT_NAME_OID, Der.octetString(Der.sequence(*sans.toTypedArray()))),
            )),
        )
    ...
    fun buildSelfSigned(
        ...
        sanDnsNames: List<String> = emptyList(),
        sanIpAddresses: List<InetAddress> = emptyList(),
    ): X509Certificate = buildSelfSigned(
        keyPair = keyPair,
