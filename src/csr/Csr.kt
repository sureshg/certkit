...
import dev.suresh.certkit.pem.PemType
import dev.suresh.certkit.pem.encodePem
...
object Csr {
    ...
    fun findSignatureAlgorithm(name: String): SignatureAlgorithmId =
        signatureAlgorithms[name]
            ?: throw IllegalArgumentException("Unknown signature algorithm '$name'")

    /** Creates a CSR from an X.500 name string, signature algorithm name, and key pair. */
    fun create(x500Name: String, signatureAlgorithm: String, keyPair: KeyPair): CertificationRequest {
    ...
    fun create(
        ...
        CertificationRequest(info, algorithmId, signature.copyOf())
}

/** Signature algorithm identifier resolved from JCA security providers. */
data class SignatureAlgorithmId(val name: String, val oid: String) {
...
data class CertificationRequestInfo(val subject: X500Principal, val publicKey: PublicKey) {
    ...
    val encoded: ByteArray = Der.sequence(
        VERSION_0, subject.encoded, publicKey.encoded, EMPTY_ATTRIBUTES,
    )
    ...
    fun sign(algorithmId: SignatureAlgorithmId, privateKey: PrivateKey): ByteArray =
        Signature.getInstance(algorithmId.name).apply {
            initSign(privateKey)
            update(encoded)
        }.sign()
}
...
class CertificationRequest(
    ...
    val encoded: ByteArray = Der.sequence(
        info.encoded,
        Der.sequence(algorithmId.encoded),
        Der.bitString(0, signature),
    )
