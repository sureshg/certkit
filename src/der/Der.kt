...
object Der {
    ...
    fun utcTime(value: Instant): ByteArray =
        tag(UTC_TIME_TAG, value.toLocalDateTime(TimeZone.UTC).format(UTC_TIME_FORMAT).encodeToByteArray())
     ...
     * Long form: first byte = 0x80 | numBytes, followed by numBytes of big-endian length.
     */
    private fun decodeLengthAt(buffer: ByteArray, offset: Int): Pair<Int, Int> {
        ...
        val length = (0 until numBytes).fold(0) { acc, i ->
     ...
     *
     * Short form (< 128): single byte.
     * Long form: lead byte with high bit set + number of length bytes, then big-endian length.
     */
